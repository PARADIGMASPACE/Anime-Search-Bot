...
async def show_favorites(message: types.Message, lang: str = None):
    ...
    logger.info(
        f"Handler 'show_favorites' started | user_id: {user_id} | username: @{username}"
    )
    ...
    try:
        ...
        if cached_favorites:
            ...
            if favorites_list:
                ...
                logger.debug(
                    f"Cached new favorites | user_id: {user_id} | count: {len(favorites_list)}"
                )
        ...
        logger.error(
            f"Handler 'show_favorites' failed | user_id: {user_id} | error: {e}"
        )
...
async def add_favorite(callback: types.CallbackQuery, lang: str = None):
    ...
    try:
        ...
        if not cached_anime:
            await callback.answer(
                i18n.t("favorites.not_found", lang=lang), show_alert=True
            )
            return
        ...
        if not anime_id:
            ...
            if not data:
                await callback.answer(
                    i18n.t("favorites.error", lang=lang), show_alert=True
                )
                return
            logger.debug(f"{data}")
        ...
        if is_already_favorite:
            await callback.answer(
                i18n.t("favorites.error_added", lang=lang), show_alert=True
            )
            return
        await add_favorite_anime_user(anime_id, user_id)
        await favorite_cache.invalidate_user_favorites(user_id)

        keyboard = get_anime_menu_keyboard(
            shikimori_id, is_favorite=True, lang=lang, anime_id=anime_id
        )
        await callback.message.edit_reply_markup(reply_markup=keyboard)
        await callback.answer(i18n.t("favorites.added", lang=lang))
        logger.info(
            f"Handler 'add_favorite' completed | user_id: {user_id} | shikimori_id: {shikimori_id}"
        )
    except Exception as e:
        logger.error(
            f"Handler 'add_favorite' failed | user_id: {user_id} | error: {e}\n{traceback.format_exc()}"
        )
    ...
    try:
        ...
        if callback.message.photo:
            if shikimori_id is None:
                shikimori_id = 0
            keyboard = get_anime_menu_keyboard(
                shikimori_id, is_favorite=False, lang=lang, anime_id=anime_id
            )
            await callback.message.edit_reply_markup(reply_markup=keyboard)
...
async def clear_favorites(callback: types.CallbackQuery, lang: str = None):
    ...
    try:
        ...
        logger.error(
            f"Handler 'clear_favorites' failed | user_id: {user_id} | error: {e}"
        )
